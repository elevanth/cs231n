...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    affine1, cache1 = affine_relu_forward(conv, W2, b2)
    scores, cache2 = affine_forward(affine1, W3, b3)
    ...
    dx, dw3, db3 = affine_backward(dx, cache2)
    grads['W3'] = dw3 + self.reg * self.params['W3']
    grads['b3'] = db3
    loss += 0.5 * self.reg *np.sum(W3 ** 2)
    
    dx, dw2, db2 = affine_relu_backward(dx, cache1)
    grads['W2'] = dw2 + self.reg * self.params['W2']
    grads['b2'] = db2
    loss += 0.5 * self.reg *np.sum(W2 ** 2)
    
    dx, dw1, db1 = conv_relu_pool_backward(dx, cache_conv)
